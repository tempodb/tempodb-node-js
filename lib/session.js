...
Session.prototype.doRequest = function(method, path, queryParams, body, callback, cursored) {
    ...
    var wrapped = function(resp) {
      ...
      if (resp.status < 300) {
        ...
        var err = new Error('Response returned non-200 code');
        err.response = resp
        cb(err, null)
      }
    ...
    var req = this.connection.request(options, function (res) {
        ...
        if(res.headers['content-encoding'] == 'gzip') {
          ...
          gunzip.on('data', function(data) {
            ...
            var respObj = new response.Response(res, buffer.join(''), session, cursored);
            if (statusc < 300) {
    ...
    req.end();
    return deferred.promise.then(wrapped, wrapped);
}
